...
function encodeAdbQuery(service: string): Uint8Array {
...
type LogLevel = "info" | "pass" | "fail" | "data" | "heading";
...
async function testBridgeAvailability(): Promise<boolean> {
...
async function testVersionQuery(): Promise<void> {
    log("── Test 2: host:version query ──", "heading");
    const bridge = window.__mbfBridge!;
    let conn: MbfAdbConnection | undefined;
    try {
        ...
        let resolve!: () => void;
        const firstChunk = new Promise<void>((r) => { resolve = r; });
        ...
        if (!ok) { log("FAIL: write returned false", "fail"); return; }
        ...
        log(`FAIL: ${(e as Error).message}`, "fail");
    } finally {
        await conn?.close();
    }
}
...
async function testMultipleConnections(): Promise<void> {
    ...
    const bridge = window.__mbfBridge!;
    let c1: MbfAdbConnection | undefined;
    let c2: MbfAdbConnection | undefined;
    try {
...
async function testFlowControl(): Promise<void> {
...
async function testCloseIdempotency(): Promise<void> {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (window.__mbfBridge?.isAvailable) {
